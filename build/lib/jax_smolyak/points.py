...
class Leja(Points):
    ...
    def get_random(self) : #get_random(self, shape=(1,)):
        if self.domain is None :
...
class Multi :
    ...
    def get_random(self, n: int = 0) -> np.ndarray: 
        #This should become vectorized!, e.g. allow providing a shape, and in 1D, default the shape to (1,)
        if n == 0 : return np.array([g.get_random() for g in self.gs]) #g.get_random(shape=(self.d,))
        return np.array([[g.get_random() for g in self.gs] for _ in range(n)]) g.get_random(shape=(n, self.d,))
