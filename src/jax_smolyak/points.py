...
class Points(ABC):
    ...
    @abstractmethod
    def scale(self, x, m: float =None, a: float=None) -> float: ...
...
class GaussHermite(Points):
    ...
    def scale(self, x: ArrayLike, m: float = None, a: float = None) -> ArrayLike:
        """
    ...
    def scale_back(self, x: float) -> ArrayLike:
...
class Leja(Points):
    ...
    def scale(self, x, d1=None, d2=None) -> ArrayLike:
        """
    ...
    def get_random(self) -> float:  # get_random(self, shape=(1,)):
        if self.domain is None:
...
class Multi:
    ...
    def __init__(self, gs: Iterable[Points]) -> None:
        self.gs = gs
        self.d = len(self.gs)
        self._is_nested = gs[0].is_nested

    def __getitem__(self, i: int) -> Points:
        return self.gs[i]
    ...
    def get_random(self, n: int = 0) -> np.ndarray:
        # This should become vectorized!, e.g. allow providing a shape, and in 1D, 
        # default the shape to (1,)
        if n == 0:
            return np.array(
                [g.get_random() for g in self.gs]
            )  # g.get_random(shape=(self.d,))
        return np.array(
            [[g.get_random() for g in self.gs] for _ in range(n)]
        )  # g.get_random(shape=(n, self.d,))

    def scale(self, x: float) -> np.ndarray:
        return np.array([g.scale(xi) for g, xi in zip(self.gs, x)])

    def scale_back(self, x) -> np.ndarray:
        return np.array([g.scale_back(xi) for g, xi in zip(self.gs, x)])


class GaussHermiteMulti(Multi):
    def __init__(self, mlist: Iterable[float], alist: Iterable[float]) -> None:
        super().__init__([GaussHermite(m, a) for m, a in zip(mlist, alist)])

    def __repr__(self):
        # turn into a string representation of the object. a `repr`
        #FIXME with forming an actual string, returning the string
...
class LejaMulti(Multi):
    def __init__(self, *, domains=None, d=None):
        if domains is not None:
            super().__init__([Leja(domain) for domain in domains])
        elif d is not None:
            super().__init__([Leja() for _ in range(d)])
        else:
            raise
