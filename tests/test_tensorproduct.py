...
def generate_test_function(ip, n=1) :
    ...
    coeffs /= sum(coeffs)
    idxs    = np.array([np.random.randint(low=0, high=d+1, size=n) for d in ip.degrees]).T

    print(f'Generating random polynomial as a weighted sum of {n} basis polynomials in {len(ip.degrees)} dimensions.\n' +
          'Weights:\n', coeffs, '\nDegrees:\n', idxs)
    ...
    if isinstance(ip.gens, points.LejaMulti) :
        ...
        def test_f(x, coeffs, degrees, gen) :
            ...
            res = 0
            for c, idx in zip(coeffs, idxs) :
                res += c * np.prod([np.polynomial.hermite.Hermite([0]*i + [1])(xi) for i, xi in zip(idx, x)])
            return res
