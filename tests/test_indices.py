...
def get_random_indexsets(nested=False) :
    ...
    b = .1 + 1.9 * np.random.random()  # b \in [0.1,3)
    ...
    print(f'\tConstructed {d}-dimensional multi-index sets with a={a}, b={b} and target n={n_t}. ' +
...
def test_smolyak_coefficients() :
    ...
    for i in range(10) :
        ...
        for nu_1, nu_2 in zip(isparse, idense):
            c_1 = smolyak_coefficient_zeta_sparse(lambda j : k[j], l, nu=nu_1, cutoff=len(k))
